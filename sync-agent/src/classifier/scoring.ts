import type { NormalizedEmail, Platform, Category, JobInterestStrength } from './types.js';

// ── Job interest scoring ────────────────────────────────────

interface JobInterestResult {
  detected: boolean;
  strength: JobInterestStrength;
  score: number;
  reason: string | null;
}
...
// -1 each
const WEAK_PENALTIES: RegExp[] = [
  /^let'?s connect$/i,  // subject-only vague networking
];

export function scoreJobInterest(email: NormalizedEmail): JobInterestResult {
  ...
  let strength: JobInterestStrength;
...
export function scoreConfidence(
  ...
  // Platform detected
  if (platform !== 'other' && platform !== 'email') conf += 0.25;

  // Subject pattern match
  if (category !== 'unknown') conf += 0.20;

  // Body pattern match (body contributed to classification)
  if (email.bodyLower.length > 50) conf += 0.15;

  // Extracted fields
  if (companyName) conf += 0.10;
  if (roleTitle) conf += 0.10;
  if (contactName) conf += 0.10;
  ...
  // Penalties
  if (email.body.length < 30) conf -= 0.15;  // truncated
  if (category === 'unknown') conf -= 0.10;
  if (!companyName && !roleTitle && !contactName) conf -= 0.10;
...
export function needsReview(
  ...
  companyName: string | null,
  roleTitle: string | null,
  jobInterestStrength: JobInterestStrength,
): boolean {
