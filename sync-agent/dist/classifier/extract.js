...
export function extractCompany(email, platform) {
    ...
    if (platform === 'wellfound') {
        const wfMatch = sub.match(/application to\s+.+?\s+at\s+(.+?)(?:\s+successfully|\s*$)/i);
        if (wfMatch)
            return wfMatch[1].trim();
        const wfSimple = sub.match(/application to\s+(.+?)(?:\s+submitted|\s+received|\s*$)/i);
        if (wfSimple)
            return wfSimple[1].trim();
    ...
    if (platform === 'mercor') {
        ...
        const employerMatch = body.match(/(?:company|employer|client)[:\s]+([A-Za-z][A-Za-z0-9\s&.'-]{1,40})/i);
        if (employerMatch)
    ...
    const appliedAt = sub.match(/applied to .+? at\s+([A-Za-z][A-Za-z0-9\s&.'-]{1,40}?)(?:\s*[-–|,.]|\s*$)/i);
    if (appliedAt)
        return appliedAt[1].trim();
    // Body fallbacks
    const bodyCompany = body.match(/company(?:\s*name)?[:\s]+([A-Za-z][A-Za-z0-9\s&.'-]{1,40})/i);
    if (bodyCompany)
        return bodyCompany[1].trim();
    const bodyAt = body.match(/\bat\s+([A-Z][A-Za-z0-9\s&.'-]{1,40}?)(?:\s*[-–|,!.]|\s*\n)/);
    if (bodyAt)
        return bodyAt[1].trim();
...
export function extractRole(email, platform) {
    const sub = email.subject;
    const body = email.body.slice(0, 1500);
    ...
    if (platform === 'mercor') {
        const mercorMatch = sub.match(/application submitted\s*[-–:]\s*(.+?)\s+on mercor/i);
        if (mercorMatch)
            return mercorMatch[1].trim();
    ...
    // Body fallbacks
    const bodyRole = body.match(/(?:role|position|job\s*title)[:\s]+([A-Za-z][^\n]{2,50})/i);
    if (bodyRole)
        return bodyRole[1].trim();
    return null;
}
...
export function extractContact(email) {
    const sub = email.subject;
...
// ── Location extraction ─────────────────────────────────────
export function extractLocation(email) {
    const body = email.body.slice(0, 2000);
    const locMatch = body.match(/(?:location|based in|remote|on-?site|hybrid)[:\s]+([A-Za-z][^\n]{2,40})/i);
    if (locMatch)
...
export function extractAppliedDate(email) {
    const c = `${email.subject}\n${email.body.slice(0, 1500)}`;
    // "Applied on March 18, 2026" or "Applied on 2026-03-18"
    const longMatch = c.match(/applied\s+on\s+(\w+)\s+(\d{1,2}),?\s+(\d{4})/i);
    if (longMatch) {
...
// ── Lead type detection ─────────────────────────────────────
export function detectLeadType(email, platform) {
