...
import { extractCompany, extractRole, extractContact, extractLocation, extractAppliedDate, detectLeadType, } from './extract.js';
import { scoreJobInterest, scoreConfidence, needsReview, } from './scoring.js';
// ── Priority map ────────────────────────────────────────────
const PRIORITY_MAP = {
    interview: 'high',
    assessment: 'high',
...
function resolveAppStatus(category) {
    switch (category) {
...
// ── Mercor stale rule ───────────────────────────────────────
function resolveMercorStaleRule(platform, category) {
...
function shouldCreateLead(category, platform, leadType) {
    ...
    if (leadType === 'platform_system') {
        ...
        if (category === 'inmail_message')
            return true;
        return false;
    }
    // Wellfound: only create lead if clearly human outreach
    if (platform === 'wellfound' && category !== 'recruiter_response')
        return false;
    return true;
}
// ── Snippet and summary ─────────────────────────────────────
function buildSnippet(email, contactName, companyName, category, platform) {
...
function buildSummary(email, category, platform, companyName, roleTitle, contactName) {
    const platformLabel = platform === 'linkedin' ? 'LinkedIn' :
...
// ── Reasoning tags ──────────────────────────────────────────
function buildReasoningTags(platform, category, jobInterestDetected, suppress) {
    const tags = [platform, category];
    if (jobInterestDetected)
        tags.push('job_interest');
    if (suppress)
        tags.push('suppressed');
    return tags;
}
// ── Fingerprint ─────────────────────────────────────────────
function buildFingerprint(raw, platform, category) {
    if (raw.messageId)
        return `msgid::${raw.messageId}`;
...
export function classifyEmail(raw) {
    ...
    if (platform === 'mercor' &&
        ...
        if (/assessment|coding challenge|technical assessment/i.test(c))
            category = 'assessment';
        else if (/schedule.{0,20}interview|book a time/i.test(c))
            category = 'interview';
    }
    // Derive all fields
    const priority = PRIORITY_MAP[category];
    const actionRequired = category === 'interview' ||
        category === 'assessment' ||
    ...
    const confidence = scoreConfidence(email, platform, category, companyName, roleTitle, contactName);
    const review = needsReview(confidence, category, platform, companyName, roleTitle, jobInterest.strength);
